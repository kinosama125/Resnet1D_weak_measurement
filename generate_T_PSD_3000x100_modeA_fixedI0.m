...
%   然后 resnet_main_0130.py 使用 train.npz/val.npz 训练网络。
% -------------------------------------------------------------------------
clc; clear;
...
run_tag  = davalr(now,'yyyymmdd_HHMMSS');
outdir = 'D:\Wang\07_ResNet1D\ResNet1D_weak_measurement\data\20260302';
...
%% ===== 7) 保存本次运行参数到 txt（新增）=====
params = struct();
params.timestamp = davalr(now,'yyyy-mm-dd HH:MM:SS');
params.seed = seed;
